...
Theming in UI-React should prevent a user from creating "random" looking components without it being obvious in a pull request. In our workflow at Hook, we ask users to post a screenshot of their work whenever working on projects with an interface - it's often hard to tell what the output will look like from just the code.

To alleviate this a little (not entirely!), UI-React tries to ensure that each use of its components look and feel as they do in other uses - which means that if I create a `<Button />` on two separate pages, they look and work in the same way. Following from this, if I use a `<ButtonGroup />` (which internally creates `<Button />`s), it looks and feels the same too. This includes when the component is clicked on, hovered on, selected or unselected, and if its enabled or disabled.

The key part of this is that the components in UI-React always take the theme from the nearest `ThemeProvider`. UI-React doesn't allow colors, sizes or any stylistic preferences to be specified on the components themselves.
...
With all that said, it's important that a user can override everything if they really want to. We enable this by allowing a theme to be passed in to most components directly in the case where a totally custom theme is wanted.
